...
function_labeled = f''' You will be provided with a Java function name as well as the java source code where it is being used. 
The query will be limited with {delimiter} characters
Then classify each class name in any of the 31 different types in the context of the source code, it must be the best fit. 
Types: Creation, Retrieval, Modification, Deletion, Operation, Calculation, Transformation, Validation, Initialization, Cleanup, Parsing, Formatting, Conversion, Generation, Sorting, Searching, Filtering, Aggregation, Mapping, Iteration, Callback, Initialization, Termination, Serialization, Deserialization, Encryption, Decryption, Authentication, Authorization, Error Handling, Logging, Validation, Normalization, Denormalization, Caching, Configuration, Mapping, Migration, Integration, Optimization, Synchronization, Asynchronization, Monitoring, Notification, Backup, Restore, Compression, Decompression. 
...
for json_file in json_files:
    ...
    java_file = "java_files/" + filename + ".java"
    ...
    for key, value in function_labels.items():
        result_string = ""
        result_string += f"{java_code}\n function: {key}"
        summary_message = [
            {'role': 'system',
    ...
    for key, value in class_names.items():
        ...
        print(f"{key}:", response)
        class_names[key] = response
    complete_data[filename] = {
    "imports_documentation": import_dictionary,
    "classes_labeled": class_names,
    "function_summaries": functions,
    "functions_labeled" : function_labels}
